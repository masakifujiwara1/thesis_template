...
\section{本章の概要}
% 本章では，移動ロボットのナビゲーションで予測結果を利用する実験を行い，その結果を評価する．
% なお，実験では歩行者の予測した軌道を，どのように活用するかを等速で往復する人のモデルを対象に議論する．
...
\section{実験概要}
予測結果を応用したナビゲーションシステムによって，移動ロボットの挙動がどのように変化するかを観察・評価するための実験を行う．実験では，3つのナビゲーションシナリオで移動ロボットを自律走行させ，各シナリオにつき10回ずつ走行を実施してデータを取得した．
...
実験は\figref{Fig:sim-env}に示すように，Gazebo\cite{Gazebo62:online}のシミュレータ環境上で行う．また，\figref{Fig:sim-robot}に示すように，シミュレータで再現した移動ロボット（ORNE-box2\cite{井口颯人2023屋外自律移動ロボットプラットフォーム-orne}）を用い，\figref{Fig:sim-actor}に示す歩行者はGazeboのプラグイン\cite{Actors-G87:online}を使用してシミュレータ内に配置した．歩行者の動きには，Social Force Model\cite{s-force-model}のプラグイン\cite{techlife87:online-sfm-plugin}を使用した．なお，速度のみ$0.6m/s$に変更して実験を行った．
...
まず，\figref{Fig:scenario1-result}に示すシナリオ1の結果を比較すると，走行時間が約78％，走行距離が約3％悪化した一方で，最小距離が約29％，最小TTCが約55％ほど大幅に改善した．
次に，\figref{Fig:scenario2-result}に示すシナリオ2の結果では，走行時間が約54％，走行距離が約4％悪化したが，最小距離が約10％，最小TTCが約14％改善した．
% \figref{Fig:scenario2-result}に示すシナリオ3の結果では，走行時間が約57％，走行距離が約1％増加，歩行者2との最小距離が約17％減少したが，歩行者1との最小距離が，歩行者1との最小TTCが約1.3倍，歩行者2との最小TTCが約1.1倍と改善した．
\figref{Fig:scenario3-result}に示すシナリオ3の結果では，走行時間が約57％，歩行者2との最小TTCが約15％悪化した．それ以外の項目は，最大で約2％ほどの変化しか生じなかった．

これらの結果から，予測結果を用いることで，ロボットのナビゲーションにおいて接触リスクが低減される可能性が示された．一方で，効率性の面では課題が残ることや，複雑なシナリオでは接触リスクの低減が困難であることも確認された．
...
シナリオ1とシナリオ2では，歩行者との最小距離と最小TTCが改善されており，予測結果を活用することで早期に停止を行い，歩行者との接触リスクを低減できる一方，停止や回避行動に伴う走行時間・距離の増加が生じたと考えられる．
% シナリオ2が，シナリオ1と比べて最小距離と最小TTCの改善幅が小さかった．これは，シナリオ2は狭い通路での歩行者とのすれ違いであることと，歩行者が接近する方向によって
シナリオ3では，歩行者1に対する最小距離と最小TTCが改善されているが，歩行者2との最小TTCが悪化している．これは，狭い通路でのすれ違い時にロボットと歩行者2の接近速度が増加していることを意味している．この結果から，狭い空間などの複雑なシナリオでのナビゲーションにおいては，予測結果の精度や扱い方に改善の余地があると考えられる．また，本研究の学習に用いたデータセットはいずれも屋外の広い空間のデータであり，狭い環境での学習サンプルが不足していたことも一因と考えられる．

本実験で用いたシミュレータの歩行者は，動かすアルゴリズムにSocial Force Model\cite{s-force-model}を用いている．このモデルによって，ロボットと歩行者間や歩行者同士の相互作用を再現しているが，現実の歩行者の動きとは異なっている．その結果，現実の歩行者の動きを重視して学習したモデルと実験環境の性質が乖離し，予測性能が低下した可能性がある．さらに，予測結果を用いる場合の標準偏差が大きい要因として，以下の点が考えられる．
\begin{itemize}
...
\end{itemize}
これらの課題を解決するためには，狭い空間など多様な環境，相互作用を含むデータセットの活用や，提案したナビゲーションシステムの予測結果の扱い方の改善などが必要になると考えられる．
