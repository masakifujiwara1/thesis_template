...
シナリオ1とシナリオ2では，歩行者との最小距離と最小TTCが改善されており，予測結果を活用することでロボットが早期に停止し，歩行者との接触リスクを低減できる可能性を示している．一方，停止や回避行動に伴い，走行時間・距離の増加が生じたと考えられる．付録Aの\figref{Fig:cross-viz}および\figref{Fig:block-viz}には，ロボットと歩行者の位置の時間変化を可視化した例を示している．これらの図から，ロボットが早期に減速または停止を行う様子が確認できる．
...
\begin{itemize}
  ...
  \item 0.4秒ごとに更新される予測結果がロボットの経路計画にチャタリングを引き起こす
...
%     \centering
%     \includegraphics[keepaspectratio, scale=0.75]{images/hallway_normal.pdf}
%     \caption{hallway normal a}
%     \label{Fig:hallway-normal}
%   \end{subfigure}
%   \vfill
%   \begin{subfigure}{0.8\textwidth}
%     \centering
%     \includegraphics[keepaspectratio, scale=0.75]{images/hallway_normal.pdf}
%     \caption{hallway normal b}
%     \label{Fig:hallway-pred}
%   \end{subfigure}
%   \caption{Hallway normal scenarios}
%   \label{Fig:hallway-viz}
% \end{figure}
