...
\figref{Fig:global-costmap}に示すように，グローバルコストマップに独自のレイヤであるMarker Array Layerを追加した．このレイヤで予測結果を受け取り，コストマップに反映する．\figref{Fig:costmap-flow}のように，各レイヤでの処理を重ねて合わせて，最終的なグローバルコストマップを生成する．グローバルコストマップを拡張したのは，\ref{sec:navigation-stack}節で述べたように，多くのロボットで使用されているNavigation Stackで予測結果を容易に利用できるようにするためである．
